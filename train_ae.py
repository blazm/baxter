...
from models import build_conv_ae, build_mnist_ae
...
if __name__ == "__main__":
    ...
    generator_mode = (parameters["dataset"]["generator_mode"])
    ...
    experiment_label = "{}.{}.e{}.bs{}.lat{}.c{}.f{}.opt-{}.loss-{}".format(
        model_label, generator_mode, num_epochs, batch_size,
    ...
    if do_train:
        ...
        if train_dir is not None:
            ...
            #fitting_generator = data_generator(train_dir, train_images, img_shape=img_shape,  batch_size=batch_size, mode=generator_mode)

            fitting_generator = random_data_generator(dir_with_src_images, base_image, object_images, img_shape=img_shape, batch_size=batch_size, mode=generator_mode)
        ...
        if valid_dir is not None:
            ...
            valid_generator = random_data_generator(dir_with_src_images, base_image, object_images, img_shape=img_shape, batch_size=batch_size, mode=generator_mode)
        else: # if no data, then use mnist
        ...
        csv = CSVLogger('snapshots/{}.csv'.format(experiment_label), append=True, separator=',')
        callbacks=[tb, csv]
        steps = np.ceil(total_images // batch_size)
    ...
    if do_test:
        ...
        if test_dir is not None:
            test_images = list_images_recursively(test_dir)
            print("Total valid images: ", len(test_images), test_images[0])
            if interactive:
                input("Press any key...")
            valid_generator = data_generator(test_dir, test_images, img_shape=img_shape, batch_size=batch_size, mode=generator_mode)
            h,w,ch = img_shape
